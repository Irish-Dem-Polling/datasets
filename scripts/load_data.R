...
dat_ipi_subset <- select(dat_ipi, pollster, 
                         date, date_start, 
...
dat_redc_props <- dat_redc_clean_joined %>% 
    ...
    select(date, date_start, date_end, date_middle, 
...
dat_banda_govsat_clean <- dat_banda_govsat %>% 
    ...
    rename(satisfaction_government = Opinion,
           date_middle = date)
...
dat_banda_leaders_clean_joined <- dat_banda_leaders_clean_joined %>% 
    rename(satisfaction_leader = Opinion,
